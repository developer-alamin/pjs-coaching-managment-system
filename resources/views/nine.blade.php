...
<div class="modal fade" id="nineUpdateModal" tabindex="-1" aria-labelledby="exampleModalLabel"
  data-bs-backdrop="static" aria-hidden="true">
  <div class="modal-dialog modal-lg ">
    <div class="modal-content">
      <div class="modal-header">
        <h6>PJS Coahing Center Seven Student Data Show</h6>
        <div class="nineEditIdDiv">
          <h4></h4>
        </div>
      </div>
...
function getNine() {
          ...
          if (response.status == 200) {
            ...
            $.each(getData, function (i) {
             var id = "<td>" + getData[i].id + "</td>";
              ...
              $('<tr>').html(name+fname+mname+studentId+phone+post+category+classs+taka+img+edit+deleted).appendTo('.nineTbody');

            });

            $(".editButton").click(function() {
                var id = $(this).data('edit');
                $("#nineUpdateModal").modal('show');
                $(".nineEditIdDiv h4").html(id);
                nineEditShow(id);
            });
  ...
  function nineEditShow(id) {
   var data = {id:id}
  var url = "/nineEditShow";
  axios.post(url, data)
  .then(function(response) {
    if (response.status == 200) {
...
function nineDelete(deleteId) {
    ...
    swal({
      title: "Are you sure?",
      text: "Are You Want To Nine Student "+deleteId+" Number Data Deleted!",
      icon: "warning",
      buttons: true,
      dangerMode: true
    })
