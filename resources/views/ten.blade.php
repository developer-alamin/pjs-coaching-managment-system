@extends('admin.app')
@section('title','Admin | Ten')
@section('content')
	<div class="tenRootDiv">
		<div class="tenTitleDiv">
			<h4 class="tenTitle">Ten Student Table</h4>
			 <img src="{{asset('img/pjs.jpg')}}" class="previewImg">
		</div>
		<div class="tenTableDiv">
...
<div class="modal fade" id="tenUpdateModal" tabindex="-1" aria-labelledby="exampleModalLabel"
  data-bs-backdrop="static" aria-hidden="true">
  <div class="modal-dialog modal-lg ">
    <div class="modal-content">
      <div class="modal-header">
        <h6>PJS Coahing Center Seven Student Data Show</h6>
        <div class="tenEditIdDiv">
          <h4></h4>
        </div>
      </div>
          ...
          if (response.status == 200) {
            ...
            $.each(getData, function (i) {
             var id = "<td>" + getData[i].id + "</td>";
              ...
              $('<tr>').html(name+fname+mname+studentId+phone+post+category+classs+taka+img+edit+deleted).appendTo('.tenTbody');

            });

             $(".editButton").click(function() {
                var id = $(this).data('edit');
                $("#tenUpdateModal").modal('show');
                $(".tenEditIdDiv h4").html(id);
                tenEditShow(id);
            });
...
function tenDelete(deleteId) {
    ...
    swal({
      title: "Are you sure?",
      text: "Are You Want To Ten Student "+deleteId+" Number Data Deleted!",
      icon: "warning",
      buttons: true,
      dangerMode: true
    })
