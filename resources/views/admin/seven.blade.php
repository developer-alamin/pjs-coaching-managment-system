@extends('backend.app')
@section('title','Admin | Seven')
@section('content')
<br>
<div class="sevenStudentData">
  <div class="card">
    <div class="card-header">
      <strong>PJS Seven Student Data</strong>
      <img src="{{asset('img/pjs.jpg')}}" style="float: right">
    </div>
    <div class="card-body">
      <table id="StudentTable" class="table table-hover table-bordered table-striped">
        <thead class="thead">
          <tr>
            <th>Name</th>
            <th>Father Name</th>
...
{{-- Seven edit modal show start form here --}}
<div class="modal fade" id="sevenUpdateModal" tabindex="-1" aria-labelledby="exampleModalLabel" data-bs-backdrop="static" aria-hidden="true">
  <div class="modal-dialog modal-lg ">
    <div class="modal-content">
      <div class="modal-header">
        <h6>PJS Coahing Center Seven Student Data Show</h6>
        <div class="SevenEditIdDiv">
          <h4></h4>
        </div>
      </div>
      ...
      <form id="UpdateForm" class="" enctype="multipart/form-data"> @csrf <div class="modal-body p-4 bg-light">
...
{{-- Seven edit modal show end form here --}}
@endsection
@section('script')
          ...
          if (response.status == 200) {
            ...
            $.each(getData, function (i) {
             var id = "<td>" + getData[i].id + "</td>";
              ...
              $('<tr>').html(name+fname+mname+studentId+phone+post+category+classs+taka+img+edit+deleted).appendTo('.sevenTbody');

            });

            $(".editButton").click(function() {
                var id = $(this).data('edit');
                $("#sevenUpdateModal").modal('show');
                $(".SevenEditIdDiv h4").html(id);
                sevenEditShow(id);
            });

            $('.deleteButton').click(function() {
                var deleteId = $(this).data('delete');
                sevenDelete(deleteId);
            });
...
function sevenEditShow(id) {
  var data = {id:id}
  var url = "/sevenEditShow";
  axios.post(url, data)
  .then(function(response) {
    if (response.status == 200) {
...
function sevenDelete(deleteId) {
    ...
    swal({
      title: "Are you sure?",
      text: "Are You Want To Seven Student "+deleteId+" Number Data Deleted!",
      icon: "warning",
      buttons: true,
      dangerMode: true
    })
