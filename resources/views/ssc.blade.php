...
<div class="modal fade" id="sscUpdateModal" tabindex="-1" aria-labelledby="exampleModalLabel"
  data-bs-backdrop="static" aria-hidden="true">
  <div class="modal-dialog modal-lg ">
    <div class="modal-content">
      <div class="modal-header">
        <h6>PJS Coahing Center Seven Student Data Show</h6>
        <div class="sscEditIdDiv">
          <h4></h4>
        </div>
      </div>
          ...
          if (response.status == 200) {
            ...
            $.each(getData, function (i) {
             ...
             var edit = "<td class='editTd'><i class='editButton fa fa-edit' data-edit='" + getData[i].id + "'></i></td>";
              ...
              $('<tr>').html(name+fname+mname+studentId+phone+post+category+classs+taka+img+edit+deleted).appendTo('.sscTbody');

            });

             $(".editButton").click(function() {
                var id = $(this).data('edit');
                $("#sscUpdateModal").modal('show');
                $(".sscEditIdDiv h4").html(id);
                sscEditShow(id);
            });
...
function sscDelete(deleteId) {
    ...
    swal({
      title: "Are you sure?",
      text: "Are You Want To SSC Student "+deleteId+" Number Data Deleted!",
      icon: "warning",
      buttons: true,
      dangerMode: true
    })
