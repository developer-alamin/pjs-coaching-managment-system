    ...
    function emailverify($token){
       ...
       if(isset($verifyToken) == true){
         ...
         if (!($studentVerify->student_email_verified_at) and $studentVerify->student_email_verified_at == null) {
             ...
             $studentVerify->save();
             return redirect(route('student.login'))->with('success', $studentVerify->student_name.' Your Email Verifyed');
         }else{
           return redirect(route('student.login'))->with('faild',$studentVerify->student_name.'Your Eamil Already Verifyed');
         }
