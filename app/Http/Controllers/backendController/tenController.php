...
class tenController extends Controller
{
    public function ten($value='')
    {
        ...
        if (Session::has('adminId')) {
           ...
           return view('admin.ten',compact('data'));
        }
    }

    public function getTen(Request $req)
    {
    	$getTen = student::where('student_class','Ten')->get();
    	return $getTen;
    }
    public function tenEditShow(Request $req)
    {
    	$id = $req->id;
     ...
     public function tenUpdate(Request $request)
    {
     ...
     public function tentDelete(Request $request)
    {
    		$deleteId = $request->id;
